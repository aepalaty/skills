...
>
> **Workflow caching tip:** I added `cache: pip` to the `setup-python` step in each workflow to speed up repeated local runs with `act` — cuts install time roughly in half on a warm cache.
